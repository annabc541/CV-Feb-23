...
#read in data
raw_dat1 = read.csv("reagents1.csv") %>% 
...
ggsave('reagents1_zero_correction_ch2.png',
       path = "~/CV Feb 23/processing/plots",
       width = 30,
       height = 12,
       units = 'cm') 
...
#read in data
raw_dat2 = read.csv("reagents2.csv") %>% 
  mutate(date = glue::glue("{Date} {Time}"),
         date = ymd_hms(date)) %>% 
  arrange(date) %>% 
  select(date,ch1 = X550.6,ch2 = X550)
...
all_dat = bind_rows(final_dat1,final_dat2) %>% 
  arrange(date)
...
#have messaged Roberto about zero disparity, waiting for an answer
#in the meantime, filter and flag data?
#could do all the above operations in one step, might look into streamlining this when I am bored?
...
hono_dat = all_dat %>% 
  mutate(reagents = ifelse(date < "2023-02-17 09:10",1,2),
         hono = ppt(ch1_ppt,ch2_ppt,se),
         date = case_when(reagents == 1 ~ date - date_corr1,
                          reagents == 2 ~ date - date_corr2))

hono_dat %>% 
  filter(date > "2023-02-26 08:00" & date < "2023-02-26 09:00") %>%
...
write.csv(processed_dat,"~/CV Feb 23/processed_data/processed_in_r.csv",row.names = FALSE)
