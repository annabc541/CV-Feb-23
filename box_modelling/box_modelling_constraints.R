...
first_day = dat_sec_since_0 %>% slice(1:24)
...
prepended_data_and_half = bind_rows(half_day,prepended_data) %>% 
...
dat_atchem2 = dat_sec_since_0 %>% 
  mutate(seconds_since_0 = seconds_since_0 + max(prepended_data_and_half$seconds_since_0) + 3600)

dat_atchem2_final = bind_rows(prepended_data_and_half,dat_atchem2)
...
for(spc in unique(atchem2_environment$variable)){
  output = atchem2_environment[atchem2_environment$variable == spc,] %>%
    select(-variable)
...
atchem2_photolysis = dat_atchem2_final %>% 
  ...
  rename_with(.cols = c(j_o1d:j_oio),
              ~ str_replace_all(.,"_",".") %>% paste0(".")) %>% 
  rename(J1 = j.o1d.,
         J2 = j.o3p.,
         J3 = j.h2o2.,
         J4 = j.no2.,
...
for(spc in unique(atchem2_photolysis$variable)){
  output = atchem2_photolysis[atchem2_photolysis$variable == spc,] %>%
    select(-variable)
